...
### v4.0.0 - Current Target Platforms: .NET 10, Spring Boot 4.1 on Java 21/25
Breaking change to the target model. Reports generated by v3.x recommended destinations that are now out of support or about to be, so the target side of every path has been re-grounded.

- **.NET 8 retired as a target.** Its support ends 10 November 2026, so it is no longer a viable destination for a programme starting now. .NET 10 (LTS, supported to approximately November 2028) is the sole .NET target. .NET 8 is still analysed in three roles: as a **detected current state** (partially modernized, but carrying its own upgrade), as a **tooling trap** (AWS Transform offers both 8 and 10 — the analyzer now instructs teams to select 10), and as a **dependency constraint** where a commercial library caps out there
- **No artificial two-step.** AWS Transform for .NET documents .NET Framework 3.5+ as a source and .NET 10 as a target, so `4.x → 10` is modelled as direct. The `3.x` pre-step is stated as verify-don't-assume rather than a mandatory hop
- **Spring targets moved to Spring Boot 4.1.x on Spring Framework 7**, replacing Boot 3. Pinned to the minor, not the major: 4.0 goes end-of-support 31 December 2026, and the ~13-month minor cadence is stated so programmes plan for 4.2/4.3 rather than treating "Boot 4" as a stable endpoint
- **Java 21 or 25**, default 25, 21 as the conservative choice. Java 17 is documented as the Framework 7 floor only — a library being "Java 17 clean" is now called out as necessary but not sufficient
- **Three-stage upgrade sequence** for Java sources (Java 8 → 21/25, Boot 2.7 → 3.5, then 3.5 → 4.1), with stage 2 mandatory and Boot 3.5 named explicitly as a transit version rather than a destination
- **Boot 4 breaking-change depth**: a 14-row cluster covering starter modularisation, Jackson 3 (`tools.jackson`), Spring Security 7 with **CSRF on API endpoints by default**, the `@MockBean` → `@MockitoBean` rename, `@SpringBootTest` no longer auto-configuring MockMvc/TestRestTemplate, Spring Batch defaulting to in-memory, Hibernate 7.4 / Spring Data JPA 4, and the OpenRewrite recipe
- **Undertow removal is a hard blocker on the WildFly path.** Boot 4 dropped Undertow support because there is no Servlet 6.1 release of it, and there is no configuration workaround — a WildFly estate must land on Tomcat 11 or Jetty 12.1
- **Namespace analysis now has three positions, not two**: `javax.*`, Jakarta EE 9/10 `jakarta.*`, and Jakarta EE 11 `jakarta.*`. "Already on jakarta" no longer means "ready"
- **Blocking vs reactive is presented as a decision, not a default.** Four steering files dropped "Reactive" from their titles and `j2ee-to-springboot-reactive.md` was renamed to `j2ee-to-springboot.md`. Both models are described with their real trade-offs and neither is ranked; the analyzer states blocking-plus-virtual-threads as its starting assumption and asks, rather than steering programmes into a paradigm shift they did not choose. Virtual-thread pinning and `ThreadLocal` behaviour are covered as findings in their own right
- **Front-end target generalised to any named framework** — React, Angular, Vue, Svelte, another framework you name, undecided, or backend-only. Previously hardcoded to React or Vue, which excluded Angular despite its higher professional usage than Vue. Added target-specific considerations for hybrid embedding into server-rendered pages, meta-framework coupling, and component-library continuity
- The framework remains your input, never a recommendation. No comparative superiority claim between front-end frameworks is encoded
